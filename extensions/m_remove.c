 ...
 *  m_remove.c: removes a user from a channel.
 *
 ...
 *  USA
 *
 *  $Id: m_remove.c 3317 2007-03-28 23:17:06Z jilles $
 */
...
m_remove(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	*buf = '\0';
	...
	if(!IsServer(source_p))
	{
		...
		/* Its a user doing a remove, but is not showing as chanop locally
		 * its also not a user ON -my- server, and the channel has a TS.
		 * There are two cases we can get to this point then...
		 *
		 *     1) connect burst is happening, and for some reason a legit
		 *        op has sent a remove, but the SJOIN hasn't happened yet or 
		 *        been seen. (who knows.. due to lag...)
		 *
		 *     2) The channel is desynced. That can STILL happen with TS
		 *        
		 *     Now, the old code roger wrote, would allow the remove to 
		 *     go through. Thats quite legit, but lets weird things like
		 *     removeS by users who appear not to be chanopped happen,
		 *     or even neater, they appear not to be on the channel.
		 *     This fits every definition of a desync, doesn't it? ;-)
		 *     So I will allow the remove, otherwise, things are MUCH worse.
	...
	if(msptr != NULL)
	{
		...
		/* jdc
		 * - In the case of a server removeing a user (i.e. CLEARCHAN),
		 *   the remove should show up as coming from the server which did
		 *   the remove.
		 * - Personally, flame and I believe that server removes shouldn't
